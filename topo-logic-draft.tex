...
% Just do correspondence between DSCs and finite sites with the BL topology.
...
Versioning parameterizations on DSCs in turn give rise to related structures on their reachable dependency posets. Since a reachable dependency poset is complete under joins, for every two event sets differing only in versions of some events, we can take the node corresponding to the union of their event sets. This yields a  endofunction on nodes of the reachable dependency poset, sending ("contracting") the lower and higher event sets both to their corresponding union.

\begin{definition}
A \textbf{poset version parameterization} is an idempotent monotone endofunction on a poset which preserves existing meets and joins.
\end{definition}

\begin{lemma}
The endofunction on a reachable dependency poset induced by a version parametrization on its underlying DSC is a poset version parameterization.
\end{lemma}
...
By Sperner's Theorem, first published in 1928, the powerset of a set with \(n\) elements, under inclusion ordering, has a width of \(\binom{n}{\floor{n/2}}\), which we denote \(\Sf(n)\). In the other extreme, when all elements of the underlying poset are strictly ordered, then all elements of the resultant downset are also strictly ordered, and hence the width is 1. This yields a key insight -- the complexity of solving a dependency problem is not strictly a function of the quantity of events -- rather, it is jointly determined by the quantity of events and the degree of dependency induced by their underlying topological structure. The general relationship is determined by the following theorem:

\begin{theorem}
For a finite poset \(P\) with \(n\) elements, the width of its free distributive lattice may be calculated as \(\Sf(n - \Mf(P))\), where \(\Mf(P)\) denotes the number of nonidempotent elements in the maximal poset version parameterization of \(P\).
\end{theorem}
